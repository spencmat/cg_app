			...
			if not fetch_err:
				for keyword in keywords:
					if keyword != wiki_page_keyword:
						paragraph, search_err = self.get_keywords_paragraph(page_html, wiki_page_keyword, keyword)
						if not search_err:
...
class LifeGeneratorService:
	...
	def _write_input_csv(self, toy_category, result_number):
		...
		with open(f"{self.lg_app_dir}/input.csv", mode='w') as lg_input_csv:
			...
			lg_input_csv_writer = csv.DictWriter(lg_input_csv, fieldnames=fieldnames, delimiter=',',
												quotechar='"', quoting=csv.QUOTE_MINIMAL)
			lg_input_csv_writer.writeheader()
			lg_input_csv_writer.writerow(dict({'input_item_type': 'toys', 'input_item_category': toy_category,
										'input_number_to_generate': result_number}))

	def _read_output_csv(self):
		lg_outputs = []
		with open(f"{self.lg_app_dir}/output.csv") as lg_output_csv:
	...
	def get_life(self, toy_category, result_number):
		...
		# return array of results read from output.csv
	...
	def get_keywords(life_results):
		ignore_words = ['aboard', 'about', 'above', 'across', 'after', 'against', 'along', 'amid', 'among', 'anti',
...
class LifeContentGeneratorApp(tk.Frame):
	...
	def submit(self):
		...
		paragraph, primary, secondary, paragraph_err = self.content_gen_service.get_content_from_list(life_keywords)

		output_rows = [dict({'input_keywords': f"{primary};{secondary}", 'output_content': paragraph})]
